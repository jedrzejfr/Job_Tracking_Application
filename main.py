...
def scrape_indeed(db_conn):
    ...
    try:
        ...
        for page in range(3):  # Scrape pages 1, 2, and 3
            ...
            try:
                job_soup = load_indeed_jobs_div(driver, "Networking", "England")
                if job_soup:
                    # Parse the page source with BeautifulSoup
                    soup = BeautifulSoup(job_soup, "html.parser")

                    # Extract job information
                    jobs_list, num_listings = extract_job_information_indeed(soup, ['titles', 'companies', 'locations', 'links', 'date_listed', 'job_ids'])
